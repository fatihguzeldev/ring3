...
The same case checks one-hop selected closures in both delay modes across three source
orders, including a selected root whose original request index is nonzero. Exact
traversal caps and owned full observations are compared after input release.
It also checks complete aggregate dependency reports and exact count/text
...
The collector reuses the same call’s prepared input, directory and address table
for dependent views; standalone readers still admit their own input. Input
admission precedes parsing; complete row and text admission precedes new owned
copies, while existing reader allocations occur earlier. All metadata,
...
pnpm typecheck
pnpm build
node tools/core/verify-wasm.mjs
```
...
coordinates, name pairing and borrowing, repeated results and input preservation.
Its outcome signatures are observed regression evidence, not a semantic oracle.
