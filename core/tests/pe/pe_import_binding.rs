...
#[test]
fn cached_addresses_are_rebound_from_explicit_lookup_tables() {
    for timestamp in [1_u32, u32::MAX] {
        let mut bytes = imported_executable::pe32(&[0xcc], "Example.dll", &["Read", "Write"]);
        bytes[1028..1032].copy_from_slice(&timestamp.to_le_bytes());
        bytes[1092..1096].copy_from_slice(&0x8000_0007_u32.to_le_bytes());
        bytes[1120..1128].fill(0xfe);
        let image = load_pe32_with_imports(&bytes, 3, |module, symbol| {
            assert_eq!(module, "Example.dll");
            match symbol {
                PeImportSymbol::ByName { name: "Read", .. } => Some(0x7000_0000),
                PeImportSymbol::Ordinal(7) => Some(0x7000_0004),
                _ => None,
            }
        })
        .unwrap();
        ...
        image.memory.read(0x0040_2060, &mut slots).unwrap();
        assert_eq!(&slots[..4], &0x7000_0000_u32.to_le_bytes());
        assert_eq!(&slots[4..], &0x7000_0004_u32.to_le_bytes());
        assert_eq!(&bytes[1120..1128], &[0xfe; 8]);
    }
}

#[test]
fn zero_oft_and_unterminated_iat_remain_rejected() {
    let mut bytes = imported_executable::pe32(&[0xcc], "Example.dll", &["Read"]);
    bytes[1124..1128].fill(1);
    assert!(load_pe32_with_imports(&bytes, 3, |_, _| Some(0x7000_0000)).is_err());
    bytes[1124..1128].fill(0);
