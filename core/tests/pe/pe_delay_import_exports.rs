use ring3_core::{
    ...
    PeDelayImportNameError, PeExportBatchError, PeExportBatchLimits, PeExportLookup,
    PeExportLookupError, PeExportNameError, PeExportSelection, PeExportTarget, PeImportLookupError,
...
fn retained_provider_matches_multiple_importers_with_fresh_limits() {
    for provider_plus in [false, true] {
        ...
        for import_plus in [false, true, false] {
            let importer = mixed_importer(import_plus);
            let expected =
                lookup_pe_delay_import_exports(&importer, 0, &provider_bytes, limits(5, 5))
                    .unwrap();
            ...
            assert_eq!(
                ...
                lookup_pe_delay_import_exports(&importer, 0, &provider_bytes, limits(5, 4))
            );
            assert_eq!(
                lookup_pe_delay_import_exports_with_provider(
                    &importer,
            ...
            assert!(matches!(
                lookup_pe_delay_import_exports_with_provider(b"", 0, &mut provider, limits(0, 0)),
                Err(PeDelayImportExportError::Imports(_))
            ));
            assert_eq!(
                lookup_pe_delay_import_exports_with_provider(
                    &importer,
            ...
            let mut invalid = importing::fixture(import_plus, 2);
            importing::put32(&mut invalid, 528, 0);
            importing::put32(&mut invalid, 544, 0);
            assert_eq!(
                lookup_pe_delay_import_exports_with_provider(
                    &invalid,
                    u16::MAX,
                ...
                Err(PeDelayImportExportError::Imports(
                    PeDelayImportLookupError::Names(
...
fn retained_provider_results_outlive_owner_with_independent_image_borrows() {
    ...
    let batch = {
        let mut provider = PeExportLookup::new(&provider_bytes);
        let batch =
            lookup_pe_delay_import_exports_with_provider(&importer, 0, &mut provider, limits(5, 5))
                .unwrap();
        assert!(
            provider
