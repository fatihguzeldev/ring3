...
mod executable;
#[path = "support/zero_constant_cases.rs"]
mod zero_constant_cases;

#[test]
fn positive_zero_preserves_lower_stack_and_control_across_budgets() {
...
use zero_constant_cases::{INPUT, OUTPUT, load, read};
...
#[test]
fn zero_push_accesses_no_data_memory_and_preserves_unrelated_cpu_state() {
    let (mut cpu, mut memory) = load(&[5.0]);
    assert_eq!(cpu.run(&mut memory, 1).instructions, 1);
    let (mut expected, mut other) = load(&[5.0, 0.0]);
...
fn each_unmasked_exception_profile_refuses_and_control_repair_retries() {
    for bit in 0..6 {
        let (mut cpu, mut memory) = load(&[7.0]);
        assert_eq!(cpu.run(&mut memory, 1).instructions, 1);
...
#[test]
fn load_zero_clears_round_up_and_preserves_sticky_exceptions_and_conditions() {
    for (comparison, condition) in [(10_f64, 0x4000), (20_f64, 0x100)] {
        ...
        code.extend([0xdf, 0xe0, 0xd9, 0xee, 0xdf, 0xe0]);
