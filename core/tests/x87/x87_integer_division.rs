use super::integer_division_cases;
...
fn precision_is_sticky_and_exact_division_clears_round_up_without_popping() {
    for profile in [0x007f, 0x027f] {
        ...
        assert_eq!(
            result(&memory),
            if profile == 0x007f {
                0x4003_b13b_2000_0000
            } else {
                0x4003_b13b_13b1_3b14
            }
        );
        cpu.eip = divide;
        refuses_unchanged(&mut cpu, &mut memory);
...
#[test]
fn single_precision_quotients_round_once_and_preserve_signed_integer_sources() {
    integer_division_cases::single_quotients_across_budgets();
}

#[test]
fn single_precision_range_is_bounded_and_fs_source_can_be_read_only() {
    for (top, source) in [
        (f64::from(f32::MIN_POSITIVE), 2),
        (-f64::from(f32::MIN_POSITIVE), 2),
        (f64::MIN_POSITIVE, 1),
        (f64::MAX, i32::MAX),
        (f64::from(f32::MAX) * 2.0, 1),
        (f64::NAN, 1),
    ] {
        let (mut cpu, mut memory) = load(top, source, SOURCE, false);
    ...
    let (mut cpu, mut memory) = load(16384.0, 32, 0xffff_ffe0, true);
    cpu.set_x87_control_word(0x007f);
    cpu.set_fs_base(SOURCE + 0x20);
    assert_eq!(cpu.run(&mut memory, 1).instructions, 1);
    memory
    ...
    assert_eq!(cpu.fs_base(), SOURCE + 0x20);
    let mut source = [0; 4];
    memory.read(u64::from(SOURCE), &mut source).unwrap();
    assert_eq!(source, 32_i32.to_le_bytes());
    memory
        .protect(0x0040_2000, 4096, Permissions::READ_WRITE)
        .unwrap();
    assert_eq!(cpu.run(&mut memory, 1).instructions, 1);
    assert_eq!(result(&memory), 512_f64.to_bits());
}

#[test]
fn integer_division_other_width_and_reverse_forms_remain_unsupported() {
    for opcode in [[0xde, 0x35], [0xda, 0x3d]] {
        let (mut cpu, mut memory) = load(6.0, 3, SOURCE, false);
        cpu.set_x87_control_word(0x007f);
        assert_eq!(cpu.run(&mut memory, 1).instructions, 1);
        memory
            ...
            .unwrap();
        memory.write(u64::from(cpu.eip), &opcode).unwrap();
        memory
