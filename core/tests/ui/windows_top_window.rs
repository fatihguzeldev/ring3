...
const TOP: u32 = 0x7000_0454;
const STACK: u32 = 0x1000_ef00;

fn query(process: &mut Process32, parent: u32) -> u32 {
    let before = process.cpu;
    process.cpu.eip = TOP;
    process.cpu.set_register(Register32::Esp, STACK);
    let frame: Vec<_> = [0x0040_10f0_u32, parent]
        .iter()
...
#[test]
fn top_window_queries_preserve_last_error_except_invalid_parent() {
    let mut process = Process32::load(&window_creation_executable::guest(), 32).unwrap();
    assert_eq!(query(&mut process, 0), 0);
    assert_eq!(
        process.run(1000).reason,
        ProcessStop::Stopped(StopReason::Breakpoint)
    );
    let hwnd = process.cpu.register(Register32::Ebx);
    assert_eq!(query(&mut process, 0), hwnd);
    assert_eq!(query(&mut process, 1), hwnd);
    assert_eq!(query(&mut process, hwnd), 0);
    assert_eq!(process.last_error().unwrap(), 0);
    assert_eq!(query(&mut process, 0xdead_beef), 0);
