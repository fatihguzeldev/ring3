...
pub const READ: u32 = 0x7000_05c4;
...
pub const COUNT: u32 = 0x0040_2300;
pub const BUFFER: u32 = 0x6000_0000;
...
pub fn process(flags: u32, bytes: &[u8]) -> Process32 {
    ...
    for index in 0..3 {
        if index == 2 {
            for value in [0, 0, 1024] {
                push(&mut code, value);
            }
            code.extend([0x56, 0xff, 0x15, 0x68, 0x20, 0x40, 0, 0x89, 0xc7]);
        }
        for value in [0, COUNT + index * 4, 512, BUFFER + index * 512] {
            push(&mut code, value);
        }
        code.extend([0x56, 0xff, 0x15, 0x64, 0x20, 0x40, 0]);
    }
    code.extend([0x89, 0xc3, 0x56, 0xff, 0x15, 0x6c, 0x20, 0x40, 0, 0xcc]);
    let exe = super::imported_executable::pe32(
        &code,
        "kErNeL32.dll",
        &["CreateFileA", "ReadFile", "SetFilePointer", "CloseHandle"],
    );
    ...
    p.memory
        ...
        .unwrap();
    p.memory
        .map_zeroed(u64::from(BUFFER), 8192, Permissions::READ_WRITE)
        .unwrap();
    p.memory
        .write(u64::from(BUFFER), &vec![0xa5; 8192])
        .unwrap();
    for address in [COUNT, COUNT + 4, COUNT + 8] {
...
pub fn bytes(p: &Process32, address: u32, length: usize) -> Vec<u8> {
...
pub fn call(p: &mut Process32, api: u32, args: &[u32]) -> u32 {
    ...
    let cleanup = if matches!(
        api,
        0x7000_0188 | 0x7000_0194 | 0x7000_0198 | 0x7000_019c | 0x7000_01a0 | 0x7000_01a4
    ) {
        4
    } else {
...
pub fn read(p: &mut Process32, handle: u32, output: u32, count: u32) -> u32 {
    call(p, READ, &[handle, output, count, COUNT, 0])
}

pub fn imported_reads_copy_real_bytes_and_stop_at_eof() {
    let source: Vec<_> = (0..513)
        .map(|i| u8::try_from((i * 37 + 11) % 256).unwrap())
        .collect();
    for flags in [0, 0x80, 0x2000_0000, 0x2000_0080] {
        for budget in [1, 5, 60] {
            let mut p = process(flags, &source);
            for (i, api) in [OPEN, READ, SEEK, CLOSE].into_iter().enumerate() {
            ...
            assert_eq!(counts, (37, 6));
            assert_eq!(p.cpu.register(Register32::Esi), FIRST);
            assert_eq!(p.cpu.register(Register32::Edi), 1024);
            assert_eq!(p.cpu.register(Register32::Ebx), 1);
            assert_eq!(p.cpu.register(Register32::Eax), 1);
            assert_eq!(p.cpu.register(Register32::Esp), 0x1001_0000);
            assert_eq!(
                [COUNT, COUNT + 4, COUNT + 8].map(|address| word(&p, address)),
                [512, 1, 0]
            );
            assert_eq!(bytes(&p, BUFFER, 513), source);
            assert_eq!(bytes(&p, BUFFER + 513, 1536 - 513), vec![0xa5; 1536 - 513]);
            assert_eq!(word(&p, ERROR), 77);
            assert_eq!(word(&p, ERRNO), 88);
            assert_eq!(open(&mut p, flags), FIRST + 4);
            assert_eq!(read(&mut p, FIRST + 4, BUFFER, 512), 1);
            assert_eq!(bytes(&p, BUFFER, 512), source[..512]);
