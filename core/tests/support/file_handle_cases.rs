...
pub fn process(flags: u32) -> Process32 {
    let mut code = Vec::new();
    for value in [u32::MAX, flags, 3, 0, 1, 0x8000_0000, PATH] {
        code.push(0x68);
        code.extend(value.to_le_bytes());
    }
    code.extend([
        0xff, 0x15, 0x60, 0x20, 0x40, 0, 0x89, 0xc6, 0x50, 0xff, 0x15, 0x64, 0x20, 0x40, 0, 0xcc,
    ]);
    let exe =
        super::imported_executable::pe32(&code, "KeRnEl32.dll", &["CreateFileA", "CloseHandle"]);
    let mut p = Process32::load_with_options(
        &exe,
        64,
        ProcessOptions {
            current_directory: b"C:\\root",
            files: &[
                FileMetadata {
                    path: b"C:\\root\\sample.bin",
                    size: 4,
                },
                FileMetadata {
                    path: b"C:\\root\\empty.bin",
                    size: 0,
                },
                FileMetadata {
                    path: b"C:\\root\\metadata.bin",
                    size: 1,
                },
            ],
            file_contents: &[
...
pub fn call(p: &mut Process32, api: u32, args: &[u32]) -> u32 {
    ...
    let cleanup = if matches!(api, 0x7000_0194 | 0x7000_019c | 0x7000_0188) {
        4
    } else {
...
pub fn args(flags: u32) -> [u32; 7] {
    [PATH, 0x8000_0000, 1, 0, 3, flags, 0]
}

pub fn imported_open_close_preserves_state_and_releases_identity() {
    for flags in [0, 0x80, 0x2000_0000, 0x2000_0080] {
        for budget in [1, 3, 30] {
            let mut p = process(flags);
            assert_eq!(word(&p, 0x0040_2060), OPEN);
            assert_eq!(word(&p, 0x0040_2064), CLOSE);
            ...
            assert_eq!(call(&mut p, OPEN, &args(flags)), FIRST + 4);
