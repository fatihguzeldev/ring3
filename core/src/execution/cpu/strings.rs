...
pub(super) fn supports_repeat(instruction: &Instruction) -> bool {
    is_comparison(instruction.code())
...
impl Cpu32 {
    ...
    pub(super) fn compare_string(
        ...
        let paired = matches!(
            instruction.code(),
            Code::Cmpsb_m8_m8 | Code::Cmpsw_m16_m16 | Code::Cmpsd_m32_m32
        );
        if instruction.op1_kind() != OpKind::MemoryESEDI
            || (paired && instruction.op0_kind() != OpKind::MemorySegESI)
        {
            return Err(StopReason::UnsupportedInstruction);
        }
        ...
        let width = match instruction.code() {
            Code::Scasb_AL_m8 | Code::Cmpsb_m8_m8 => Width::Byte,
            Code::Scasw_AX_m16 | Code::Cmpsw_m16_m16 => Width::Word,
            Code::Scasd_EAX_m32 | Code::Cmpsd_m32_m32 => Width::Dword,
            _ => return Err(StopReason::UnsupportedInstruction),
        };
        let address = self.register(Register32::Edi);
        let source = self.register(Register32::Esi);
        let left = if paired {
            let base = if instruction.segment_prefix() == Register::FS {
                self.fs_base
            } else {
                0
            };
            self.read_operand(
                Operand {
                    location: Location::Memory(base.wrapping_add(source)),
                    width,
                },
                memory,
            )?
        } else {
            self.register(Register32::Eax) & width.mask()
        };
        ...
        if repeat {
            ...
            if count > 1 && keep_scanning {
                // defer flags so a later comparison fault retains pre-instruction eflags.
                return Ok(self.eip);
            }
        }
