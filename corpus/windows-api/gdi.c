...
void entry(void) {
    ...
    void *desktop = GetDC(desktop_window);
    if (!first || !second || first == second) ExitProcess(1);
    if (!desktop_window || !desktop || desktop == first || desktop == second) ExitProcess(10);
    ...
    if (ReleaseDC((void *)123, first) || GetDeviceCaps(first, 8) != 640) ExitProcess(5);
    if (ReleaseDC(0, first) != 1 || GetDeviceCaps(first, 8) || ReleaseDC(0, first)) ExitProcess(6);
    if (GetDeviceCaps(second, 8) != 640 || ReleaseDC(0, second) != 1) ExitProcess(7);
    if (ReleaseDC(0, desktop) || GetDeviceCaps(desktop, 8) != 640 ||
        ReleaseDC(desktop_window, desktop) != 1 || GetDeviceCaps(desktop, 8)) ExitProcess(11);
