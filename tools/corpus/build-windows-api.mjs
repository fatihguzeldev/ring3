...
for (const library of ["kernel32", "user32", "gdi32", "winmm", "advapi32"]) {
  run(tools.lld, ["-flavor", "link", "/lib", "/machine:x86",
    `/def:${join(root, `corpus/windows-api/${library}.def`)}`, `/out:${library}.lib`], output);
}
for (const name of ["calls", "modules", "heap", "version", "critical-sections", "tls", "global-memory", "code-pages", "cpinfo", "wide-character", "dialog", "dialog-cbt", "messages", "get-message-wait", "translate-message", "dispatch-message", "clipboard-formats", "process-version", "metrics", "gdi", "colors", "brushes", "cursors", "cursor-position", "local-realloc", "thread-identity", "module-file-name", "resources", "string-copy", "system-directory", "interlocked", "computer-name", "mutex", "performance-clock", "current-directory", "change-directory", "find-files", "command-line", "environment", "floating-point", "millisecond-clock", "string-stores", "floating-status", "signed-products", "string-comparisons", "startup-info", "hooks", "thread-priority", "formatting", "buffer-move", "window-classes", "desktop-queries", "floating-registers", "procedures", "registry", "registry-values", "string-length", "file-attributes", "short-path", "registry-defaults", "accelerators", "window-procedures", "window-creation", "unsigned-division", "window-properties", "hook-chain", "icons", "window-messages", "path-components", "file-streams"]) {
  ...
  run(tools.lld, ["-flavor", "link", "/entry:entry",
    ...
    `/out:${name}.exe`, `${name}.obj`, ...extra, "kernel32.lib", "user32.lib", "gdi32.lib", "winmm.lib", "advapi32.lib"], output);
