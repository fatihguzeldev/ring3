...
test("tls fixtures preserve pinned object and executable bytes across builds", () => {
  ...
  try {
    const first = buildTlsFixtures(join(directory, "first"));
    const second = buildTlsFixtures(join(directory, "second"));
    ...
    for (const architecture of ["i386", "amd64"]) {
      ...
      for (const name of ["tls.s", "inspection.txt"]) {
...
test("tls source, tool, assembler, linker and byte mismatches refuse success evidence", () => {
  ...
  try {
    ...
    for (const [name, source] of [
      ...
      assert.throws(() => buildTlsFixtures(join(directory, name), { sourceDirectory: sources, contract: invalid }), /failed:/);
    }
    const wrongBytes = structuredClone(contract);
    wrongBytes.architectures.amd64.artifacts["tls.exe"].sha256 = "0".repeat(64);
    assert.throws(() => buildTlsFixtures(join(directory, "expectation"), { contract: wrongBytes }), /artifact SHA-256 mismatch/);
    const wrongDirectory = structuredClone(contract);
    wrongDirectory.architectures.amd64.directory.size += 1;
    assert.throws(() => buildTlsFixtures(join(directory, "directory"), { contract: wrongDirectory }), /LLVM TLS directory mismatch/);
...
test("tls producer preserves existing outputs and refuses linked ancestors", () => {
  const directory = mkdtempSync(new URL("paths-", outputRoot));
  const outside = mkdtempSync(join(tmpdir(), "ring3-tls-corpus-outside-"));
  try {
    writeFileSync(join(directory, "sentinel"), "existing output");
    assert.throws(() => buildTlsFixtures(directory), { code: "EEXIST" });
    assert.equal(readFileSync(join(directory, "sentinel"), "utf8"), "existing output");
    symlinkSync(outside, join(directory, "redirect"), "dir");
    assert.throws(() => buildTlsFixtures(join(directory, "redirect", "fixture")), /real directories/);
